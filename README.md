...
### Average Fare:
When we look at the average fare per ride, the totals are flipped, most likely due to the distances travelled in each city type and the mileage based total fares charged per ride. The average urban ride generates the least fare, then the suburban ride, then the rural ride, as the long distances involved in rural ride generates the highest average fare per ride. The average fare per ride for each city type is as follows:
